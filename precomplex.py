...
fragA = list(iread("pd_complex_1.xyz"))[-1]  # metal complex (fragment A)
fragB = list(iread("ar_br.xyz"))[-1]  # substrate (fragment B)

# rotate fragB +90 degrees about Y (x->z) to orient toward +Z
angle_deg = 90.0
...
fragB.set_positions(fragB.get_positions() @ rot_y.T)
...
builder = PrecomplexBuilder(
    ...
    siteA,
    siteB,
    distances=np.linspace(2.9, 3.5, 5),
)
...
# distances to sample
results = builder.build(max_keep=40, do_energy=False, max_energy_eval=40)


for i, result in enumerate(filter(lambda x: x["clash_score"] <= 0, results)):
    ...
    opt = LBFGS(merged, trajectory=f"builder_test/test_out_rotated_{i}.traj")
    # opt.run(fmax=0.50, steps=20)  # only local cleanup
    merged.set_constraint()
    write(f"builder_test/test_out_rotated_{i}.xyz", merged)
